  ...
  const cancelOrder = useCallback(
    async (LOID: string): Promise<void> => {
      ...
      const initRes = await fetch(
        `/api/jupiter/trigger-v2/orders/price/cancel/${encodeURIComponent(providerOrderId)}`,
      ...
      const confirmRes = await fetch(
        `/api/jupiter/trigger-v2/orders/price/confirm-cancel/${encodeURIComponent(providerOrderId)}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      try {
        await fetch(`/api/limit-orders/${encodeURIComponent(LOID)}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            ...(accessToken ? { Authorization: `Bearer ${accessToken}` } : {}),
